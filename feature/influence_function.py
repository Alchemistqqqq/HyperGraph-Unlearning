...
def forget_features(X, forget_ratio):
    num_nodes, num_features = X.shape
    num_to_forget = int(num_features * forget_ratio)
    for i in range(num_nodes):
        forget_indices = torch.randperm(num_features)[:num_to_forget]
        X[i, forget_indices] = 0
    return X
...
def main():
    ...
    X = forget_features(X, args['forget_ratio'])
    ...
    unlearn_grads = compute_gradients(net, X, G, lbl, train_mask, device) 
