...
def train(net, X, G, G_remain, lbl, train_mask, optimizer, epoch, device):
    ...
    # Forward pass on the updated graph (after forgetting features)
...
def main():
    ...
    best_state, best_epoch, total_training_time = train_and_evaluate_model(net, X, G, G, lbl, train_mask, val_mask, optimizer, args['num_epochs'], device)

    test_model(net, best_state, X, G, lbl, test_mask, device)
