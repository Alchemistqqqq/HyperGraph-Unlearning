...
def train_and_evaluate_model(net, X, G, lbl, train_mask, val_mask, optimizer, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 1 == 0:
            with torch.no_grad():
                val_res = infer(net, X, G, lbl, val_mask.to(device))[1]
            if val_res > best_val:
...
def test_model(net, best_state, X, G, lbl, test_mask, device):
    ...
    loss, acc = infer(net, X, G, lbl, test_mask.to(device), test=True)
    return loss, acc
...
def main():
    ...
    # Test model on original graph
    net.load_state_dict(best_state)
    print("Test before applying influence...")
    loss_before, acc_before = infer(net, X, G, lbl, test_mask, test=True)
    ...
    unlearn_grads = compute_gradients(net, X, G_remain, lbl, train_mask, device)  # Use the forgetting graph to compute gradients
    ...
    # Apply influence function to update model parameters
    apply_gradients(net, influence, scale=args['apply_scale'])

    # Test model after applying influence
