...
def main():
    ...
    with ProcessPoolExecutor(max_workers=len(devices)) as executor:
        ...
        for i, subgraph in enumerate(subgraphs):
            ...
            print(f"Scheduling training for subgraph {i + 1} on device {device}")
            subgraph_data = (
                i, subgraph, X, lbl, train_mask, val_mask, test_mask, device, args['model_class'], args['learning_rate'],
                args['weight_decay'], args['num_epochs']
            )
