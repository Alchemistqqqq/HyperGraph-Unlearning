...
def majority_voting(predictions):
    ...
    final_predictions = torch.mode(stacked_preds, dim=0)[0]
    return final_predictions

def average_voting(predictions):
    averaged_preds = torch.mean(torch.stack(predictions), dim=0)
    final_predictions = torch.argmax(averaged_preds, dim=1)
    return final_predictions

def weighted_average(predictions, weights):
    weighted_preds = torch.zeros((predictions[0].shape[0], predictions[0].max() + 1), device=predictions[0].device)
    for pred, weight in zip(predictions, weights):
        weighted_preds[torch.arange(pred.shape[0]), pred] += weight
...
def main():
    ...
    elif args['aggregation_method'] == 'majority_voting':
    ...
    final_predictions = final_predictions[:test_mask.sum().item()]
