...
def main():
    ...
    for i, subgraph in enumerate(subgraphs):
        print(f"Training subgraph {i + 1}/{len(subgraphs)}")
        subgraph_data = (i, subgraph.to(device), X.to(device), lbl.to(device), 
                         new_train_mask.to(device), val_mask.to(device), test_mask.to(device), 
                         device, args['model_class'], args['learning_rate'], args['weight_decay'], args['num_epochs'])
    ...
    final_predictions = weighted_average(all_predictions, node_coverage_weights).to(device)
